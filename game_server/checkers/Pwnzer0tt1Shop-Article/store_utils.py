...
def get_article_list():
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad article list status code: {resp.status_code}')
    return resp.json()
...
def register_user(username: str, password: str, email:str):
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad register status code: {resp.status_code}')
...
def user_info():
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad user info status code: {resp.status_code}')
    return resp.json()
...
def login_user(username: str, password: str):
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad login status code: {resp.status_code}')
    return resp.json()['user']
...
def donate_user(amount: int) -> str:
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad donate status code: {resp.status_code}')
...
def shell_article(title: str, description: str, price: float, secret: str):
    ...
    if resp.status_code != 200 and resp.status_code != 201:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad sell status code: {resp.status_code}')
...
def buy_article(article_id: int) -> bool:
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad buy status code: {resp.status_code}')
    return True
...
def logout_user() -> str:
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad logout status code: {resp.status_code}')
    return resp.json()['message']
...
def token_login(token: str):
    ...
    if resp.status_code != 200:
        checklib.quit(checklib.Status.DOWN,
                    f'Bad token login status code: {resp.status_code}')
    return resp.json()['user']
