    ...
    return (
        <Table.Tr>
            ...
            <Table.Td><Box className="center-flex-col">
                ...
                <Space h="3px" />
                <Pill style={{ backgroundColor: "var(--mantine-color-cyan-filled)", color: "white", fontWeight: "bold" }}>{round.score.team}</Pill>
            </Box></Table.Td>
            <Table.Td>
                <Box className="center-flex-row">
                    <Text className="center-flex" style={{ fontWeight: "bolder" }}>{round.score.score.toFixed(2)}</Text>
                    <DiffArrow diff={round.score.services.reduce((acc, d) => d.diff_final_score + acc, 0)} />
                </Box>
            </Table.Td>
            {services.map((service, i) => (
                <Table.Td key={i}>
                    <ServiceScoreData score={round.score.services.find(ele => ele.service === service.name)} />
                </Table.Td>
            ))}
        </Table.Tr>
    );
...
const HeaderComponent = memo(({ position, teamName, navigateBack }:{
    position?: number,
    teamName?: string,
    navigateBack: () => void
}) => (
    <Box className="center-flex">
        <Text style={{ fontWeight: "bold" }} className="center-flex" component="div">
            <FaHashtag size={20} /><Space w="xs" />{position ?? "?"}
        </Text>
        <Space w="md" />
        <Text style={{ fontWeight: "bold" }} className="center-flex" component="div">
            <MdGroups size={20} /><Space w="xs" />{teamName}
        </Text>
        <Space w="md" />
        <ActionIcon 
            variant="filled" 
...
export const ScoreboardTeamDetail = () => {
    ...
    const sortedTeamData = useMemo(() => 
        teamData.data ? [...teamData.data].sort((a, b) => b.round - a.round) : [],
        [teamData.data]
    );
        ...
        const chartData = [...teamData.data]
            ...
            .map((round) => ({
                ...
                ...round.score.services.reduce(
                    (acc, score) => ({ ...acc, [score.service]: score.final_score.toFixed(2) }),
                    {}
                )
            }));

        return {
            minPoints: min === Infinity ? 0 : Math.max(Math.round(min - min * 0.05), 0),
